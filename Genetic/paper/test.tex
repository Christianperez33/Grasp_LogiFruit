...
The objetive of the crossover or reproduction is to obtain a number of new solutions (or children) from each parent pair. To achieve this goal, each child is formed by a random fraction of the orders that compose one of the parents, and the rest of the orders are taken from the remaining parent. The number of childs that are breeded is variable, and it is passed to the algorithm as a paramater. The result of this process is a list that contains sets of solutions calles families, each one formed by a parent pair and a number of children.
